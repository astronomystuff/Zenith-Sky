...
class minimalCameraControls {
  ...
  onTouchMove(e) {
    ...
    if (this.touchMode === "pinch" && e.touches.length === 2) {
        ...
        const delta2 = pinchIntent * (-(Math.pow(1.05, -this.camera.fov)) - 0.05 * this.camera.fov);
        const delta = 2 * delta2
        this.onZoom(delta);
...
function applyAnnualAberration(raDeg, decDeg, earthVel) {
    ...
    const bx = earthVel.vx / C_AU_PER_DAY;
    const by = earthVel.vy / C_AU_PER_DAY;
    const bz = earthVel.vz / C_AU_PER_DAY;
...
function onSky3DClick(event) {
  ...
  const MAX_PLANET_DISTANCE = 0.01;
